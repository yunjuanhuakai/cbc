...
stmt (If fc expr then_ else_) =
  If fc <$> exprAndCheck expr <*> stmt then_ <*> mapM stmt else_
stmt (Switch fc expr stmts) =
  Switch fc <$> exprAndCheck expr <*> mapM stmt stmts
stmt (Case fc expr body) = Case fc <$> exprAndCheck expr <*> stmt body
stmt (For fc init cond next body) =
  For fc <$> stmt init <*> mapM exprAndCheck cond <*> stmt next <*> stmt body
stmt (While   fc cond  body ) = While fc <$> exprAndCheck cond <*> stmt body
stmt (DoWhile fc body  cond ) = DoWhile fc <$> stmt body <*> exprAndCheck cond
